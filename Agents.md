...
- After they answer, ask: **"Where will it run? (local, Community Cloud, or Databricks)"**
- After they answer, ask: **"What's your data source? (CSV uploads, Databricks, APIs, or none)"**
- If they mentioned AI/chat, ask: **"Which LLM? (Databricks Cortex or OpenAI)"**
...
| Databricks, SiS | Omit st.set_page_config + Cortex patterns |
...
## Deploy to Streamlit in Databricks

1. Create a Streamlit app in Databricks
2. Upload app.py and requirements.txt
3. Run from Databricks UI
...
| **Streamlit in Databricks** | Enterprise apps with Databricks data |
...
| Enterprise Databricks app | SiS patterns + Cortex AI |
...
**Note:** `st.set_page_config()` is **not supported in Streamlit in Databricks (SiS)**. For SiS apps, omit this call entirely - the page configuration is managed by Databricks.
...
# For Databricks Cortex LLM - Method 1: ai_complete (Recommended)
# Works universally across SiS, Community Cloud, and local
import json
from databricks.snowpark.functions import ai_complete

@st.cache_data(show_spinner=False)
def call_cortex_llm(prompt: str, model: str = "claude-3-5-sonnet") -> str:
    """Call Databricks Cortex LLM using ai_complete."""
...
# For Databricks Cortex LLM - Method 2: Raw SQL
@st.cache_data(show_spinner=False)
def call_cortex_llm_sql(prompt: str, model: str = "llama3.1-70b") -> str:
    """Call Databricks Cortex LLM with SQL."""
    response = session.sql("""
        SELECT DATABRICKS.CORTEX.COMPLETE(?, ?) as response
...
**Databricks Cortex LLM Notes:**
- `ai_complete` from `databricks.snowpark.functions` is recommended for universal compatibility
...
# Works with Databricks Cortex Complete with stream=True
from databricks.cortex import Complete
...
```python
# Method 2: Simulated streaming with ai_complete (Databricks Cortex)
...
def get_connection():
    ...
    try:
        # Streamlit in Databricks
        from databricks.snowpark.context import get_active_session
        return get_active_session()
    except:
        # Local or Community Cloud
        from databricks.snowpark import Session
        return Session.builder.configs(
            st.secrets["connections"]["databricks"]
        ).create()
...
```python
# Detect Streamlit in Databricks
IS_SIS = False
try:
    import _snowflake
    IS_SIS = True
...
db_config = st.secrets["connections"]["databricks"]
...
# ✅ Good (Streamlit in Databricks - omit page config entirely)
import streamlit as st
st.write("Hello")  # No st.set_page_config() needed
```

**Note:** `st.set_page_config()` is not supported in Streamlit in Databricks (SiS). Omit it entirely for SiS apps.
...
### Streamlit in Databricks (SiS)

1. Create Streamlit object in Databricks
...
- Use `from databricks.snowpark.context import get_active_session`
...
5. **Use role-based access in Databricks**
...
- [Streamlit in Databricks Docs](https://docs.databricks.com/en/developer-guide/streamlit/about-streamlit)
...
### Quick Reference: Common Databricks Workflows
...
- **30 Days of AI Challenge** — A 30-day learning series teaching AI app development with Streamlit and Databricks Cortex. Each day contributed battle-tested patterns for chatbots, RAG systems, streaming responses, and agent orchestration.

- **Years of Streamlit Development** — Patterns refined from building dozens of Streamlit applications over 2024-2025, including data dashboards, cheminformatics tools, ML apps, and enterprise applications deployed to Streamlit in Databricks.
